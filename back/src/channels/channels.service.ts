...
export class ChannelsService {
  ...
  async getWorkspaceChannels(url: string, myId: number) {
    return this.channelsRepository
      ...
      .innerJoinAndSelect('channels.Workspace', 'workspace', 'workapce.url === url', { url })
      .getMany();
  }

  async getWorkspaceChannel(url: string, name: string) {
    return this.workspacesRepository.findOne({ where: { name }, relations: ['Workspace'] });
  }

  async createWorkspaceChannels(url: string, name: string, myId: number) {
    const workapce = await this.workspacesRepository.findOne({ where: { url } });

    if (!workapce) return null;

    const channel = new Channels();
    channel.name = name;
    channel.WorkspaceId = workapce.id;
  ...
  async getWorkspaceChannelMembers(url: string, name: string) {
    return this.usersRepository
      ...
      .innerJoin('users.Channels', 'channels', 'channels.name === name', { name })
      .innerJoin('channels.Workspace', 'workspace', 'workapce.url = url ', { url })
      .getMany();
  }
  ...
  async createWorkspaceChannelMembers(url: string, name: string, email: string) {
    const channel = await this.channelsRepository
      .createQueryBuilder('channels')
      .innerJoin('channels.Workspace', 'workspace', 'workspcae.url =url', { url })
